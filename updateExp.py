...
class CupdateEXP:
    def __init__(self, root, year, month):
        ...
        title=Label(self.root, text=f"RTGS Cash Analysis (UPDATE EXPENTITURE RECORDS FOR THE MONTH {self.month}-{self.year})", font=("Times New Roman", 18, "bold"), bg="orange", fg="white").pack(side=TOP, fill=BOTH)
        ...
        BtnUpdate=Button(self.root, text="Save Record",  command=self.AddExpentiture, font=("Times New Roman", 13), bg="green", fg="white").place(x=680, y=260, width=120)
        BtnUpdate=Button(self.root, text="Update Record", command=self.UpdateExp, font=("Times New Roman", 13), bg="blue", fg="white").place(x=800, y=260, width=120)
    ...
    def AddExpentiture(self):
        ...
        try:
            if self.ReceptNo.get() =="" or self.Details.get() =="" or self.Date.get() =="":
                ...
                self.cur.execute(f"Select * from {self.Expperiod} where ReceptNo=?", (self.ReceptNo.get(), ))
                row = self.cur.fetchone()
                ...
                if row != None :
                    ...
                    self.cur.execute(f"Insert into {self.Expperiod} ( ReceptNo , Details , Date , Total , BF , SPORTS , GPF , EWP , ZMSEC , G7Vac , BHole , ExamTrans , FeedingP) values(?,?,?,?,?,?,?,?,?,?,?,?,?)", (
        ...
        try:
            if self.ReceptNo.get() =="" or self.Details.get() =="" or self.Date.get() =="":
                ...
                if row == None :
                    ...
                    Total_Inc= float(self.BF.get() )+ float(self.SPORTS.get())+ float(self.GPF.get())+ float(self.EWP.get())+ float(self.ZMSEC.get())+ float(self.G7Vac.get())+ float(self.BHole.get())+ float(self.ExamTrans.get())+ float(self.FeedingP.get())
                    self.Total.set(Total_Inc) 
                    self.cur.execute(f"Update  {self.Expperiod} set Details=?, Date=? , Total=? , BF=? , SPORTS=? , GPF=? , EWP=? , ZMSEC=? , G7Vac=? , BHole=? , ExamTrans=? , FeedingP=?  where ReceptNo=?", (
        ...
        try:
            if self.ReceptNo.get() =="" or self.Details.get() =="" or self.Date.get() =="":
                ...
                if row == None :
                    ...
                    if option==True:    
                        self.cur.execute(f" Delete from {self.Expperiod} where ReceptNo=?", (self.ReceptNo.get(), ))
                        self.con.commit()
